          ...
          gallery_photos: pending.map((photo) => ({
            ...
            ...(photo.cropData && {
              crop_data: {
                ...
                y: photo.cropData.originY,
                width: photo.cropData.width,
                height: photo.cropData.height,
              },
            }),
          })),
        },
        ...
        { text: "Отмена", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            try {
              await Promise.all(
                toDelete.map((id) =>
                  deleteGalleryPhoto({ userId, id: Number(id) }).unwrap(),
                ),
              );
            } catch (error) {
              toast.error(getApiErrorMessage(error, "Не удалось удалить фото"));
        ...
        {({ topInset, bottomInset }) => (
          <View className="flex-1">
            {isGalleryLoading && !photos.length ? (
              <GallerySkeleton topInset={topInset} />
