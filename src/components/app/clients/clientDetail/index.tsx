...
import { Avatar, Divider, IconButton, StSvg, Typography } from "@/src/components/ui";
...
const AppointmentRow = ({ item }: AppointmentRowProps) => {
  ...
  const extraCount =
    item.services.length + item.additional_services.length - 1;
  ...
  return (
    ...
    >
      <View className="flex-row items-center justify-between">
        <Text className="font-inter-semibold text-body text-neutral-900 flex-1 mr-2" numberOfLines={1}>
          {serviceName}
...
const StatBox = ({ label, value }: StatBoxProps) => (
  <View className="flex-1 bg-white rounded-2xl p-4 items-center gap-1">
    <Text className="font-inter-bold text-[20px] text-neutral-900">{value}</Text>
    <Text className="font-inter-regular text-caption text-neutral-500 text-center">{label}</Text>
  </View>
);
  ...
  return (
    <ScreenWithToolbar
      ...
      rightButton={
        <View className="flex-row gap-1">
          <IconButton
            icon={<StSvg name="Chart_fill" size={24} color={colors.neutral[900]} />}
            onPress={() => router.push(Routers.app.clients.statistics(customerId))}
            accessibilityLabel="Статистика"
          />
          <IconButton
            icon={<StSvg name="Wallet_fill" size={24} color={colors.neutral[900]} />}
    ...
    >
      {({ topInset, bottomInset }) => {
        if (customerLoading) {
          return (
            <View className="flex-1 items-center justify-center" style={{ marginTop: topInset }}>
              <ActivityIndicator />
            </View>
          );
        }
        ...
        if (!customer) {
          return (
            <View className="flex-1 items-center justify-center" style={{ marginTop: topInset }}>
              <Typography className="text-body text-neutral-500">Клиент не найден</Typography>
            </View>
          );
        }
            ...
            <View className="px-screen items-center gap-3">
              ...
              {customer.customer_tag && (
                <View
                  ...
                  style={{ backgroundColor: customer.customer_tag.color + "22" }}
                >
                  <Text
